...
class Model extends JOptionPane implements FunctionOfPhonebook {
  ...
  public void addContactOfPhonebook() throws IOException {
    Contact contact = new Contact.ContactBuilder()
        ...
        .lastName(
            showInputDialog(null, "Введите фамилию Контакта: ", "Окно ввода", QUESTION_MESSAGE))
        .phoneNumber(
            showInputDialog(null, "Введите номер Контакта: ", "Окно ввода", QUESTION_MESSAGE))
        .description(
            showInputDialog(null, "Введите описание Контакта: ", "Окно ввода", QUESTION_MESSAGE))
  ...
  public void findContactOfPhonebook() {
    String txt = "";
    String who = showInputDialog(null, "Введите имя контакта, которого ищете:", "Кого ищем",
        QUESTION_MESSAGE);
    for (Contact c : Phonebook.getPhonebook()) {
      if (who.equalsIgnoreCase(c.getFirstName())) {
        txt = c.getFirstName() + " " + c.getLastName() + " " + c.getPhonenumber() + " "
            + c.getDescription() + " " + c.getStatus() + "\n";
      } else {
        txt = "Не нашел";
      }
